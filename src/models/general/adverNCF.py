...
class adverNCF(AdverModel):
    ...
    def forward(self, u_ids, i_ids):
        ...
        u_ids_ = u_ids.unsqueeze(-1).repeat((1, i_ids.shape[1]))  # [batch_size, -1]

        mf_u_vectors = self.mf_u_embeddings(u_ids)
        mf_u_vectors = self.apply_filter(mf_u_vectors)
        out_u_vectors = mf_u_vectors
        mf_u_vectors = mf_u_vectors.unsqueeze(1).repeat((1, i_ids.shape[1], 1))
        
        mlp_u_vectors = self.mlp_u_embeddings(u_ids)
        #mlp_u_vectors = self.apply_filter(mlp_u_vectors)
        ...
        # user_bias = self.u_bias(u_ids_).view_as(prediction)
        # item_bias = self.i_bias(u_ids_).view_as(prediction)
        # prediction = prediction + user_bias + item_bias

        #print(mf_u_vectors.shape)

        return prediction.view(len(u_ids), -1), out_u_vectors

    def model_(self, u_ids, i_ids):
        # u_ids = torch.from_numpy(np.array(u_ids))
        # i_ids = torch.from_numpy(np.array(i_ids))
        u_ids = u_ids.repeat((1, i_ids.shape[0])).squeeze(0)
